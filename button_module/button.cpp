#include <stdbool.h>
#include <stdio.h>
#include <string.h>

#include <etl/debounce.h>

extern "C" {
#include <pico/types.h>
#include <pico/stdlib.h>
#include <pico/multicore.h>
#include <pico/i2c_slave.h>
#include <pico/rand.h>
#include <pico/rand.h>
#include <pico/time.h>

#include <hardware/gpio.h>
#include <hardware/i2c.h>

#include "../ktane-globals/def.h"
#include "button.h"
#include "display.h"
}


#define DEBUG

#ifdef DEBUG
const bool DEBUG_MODE = true;
#else
const bool DEBUG_MODE = false;
#endif

const uint8_t I2C_SLAVE_ADDRESS = 0x19;

uint8_t data = 0;

static ButtonColors button_color;
static ButtonLabels button_label;
static uint8_t battery_count;
static bool lit_frk = 0;
static bool lit_car = 0;
static uint8_t relevant_rule = 0;

static bool fail_flag = false;
static bool lose_flag = false;
static enum i2c_slave_responses i2c_state = READY_2_START;

static uint8_t recv_data = 0;


static void i2c_slave_handler(i2c_inst_t *i2c, i2c_slave_event_t event) {
    switch (event) {
        case I2C_SLAVE_RECEIVE: // Master is writing to the module
            data = i2c_read_byte_raw(i2c);
            if (recv_data) {
				if (recv_data == INIT_FRAME) { // if this is the first byte of the frame
                    battery_count = data & BATTERY_COUNT_MASK;
                    lit_frk = data & LIT_FRK_INDICATOR;
                    lit_car = data & LIT_CAR_INDICATOR;
				}
                recv_data--;
            }
            else {
                switch (data) {
                    case INIT_INFO:
                        recv_data = INIT_FRAME;
                        break;
                    case START:
                        i2c_state = (i2c_state == READY_2_START) ? RUNNING : i2c_state;
                        break;
                    case GAME_LOSS:
                        lose_flag = true;
                        break;
                    default:
                        break;
                }
            }
            break;
        case I2C_SLAVE_REQUEST: // Master is reading from the module
            i2c_write_byte_raw(i2c, i2c_state);
            i2c_state = (i2c_state == FAILED) ? RUNNING : i2c_state; // reset state to not submit multiple fails
            break;
        default:
            break;
    }
}

static void button_event(uint gpio, uint32_t events) {
    uint64_t abs_time = to_us_since_boot(get_absolute_time());
    static size_t count = 0;

    printf("button_event ran %zu times\n", count++);

    if (gpio != BUTTON) {
        printf("gpio isn't BUTTON us_since_boot: %llu\n", abs_time);
        return;
    }

    if (events & GPIO_IRQ_EDGE_RISE) {
        printf("GPIO_IRQ_EDGE_RISE us_since_boot: %llu\n", abs_time);
    }

    if (events & GPIO_IRQ_EDGE_FALL) {
        printf("GPIO_IRQ_EDGE_FALL us_since_boot: %llu\n", abs_time);
    }

    if (events & GPIO_IRQ_LEVEL_HIGH) printf("GPIO_IRQ_LEVEL_HIGH us_since_boot: %llu\n", abs_time);
    if (events & GPIO_IRQ_LEVEL_LOW) printf("GPIO_IRQ_LEVEL_LOW us_since_boot: %llu\n", abs_time);
}

void button_module_init() {
    button_color = (ButtonColors)(get_rand_64() % BTN_COLOR_NUM);
    button_label = (ButtonLabels)(get_rand_64() % LABELS_NUM);

    stdio_init_all(); // initialize stdio for printf
    
    /**************************
        GPIO init block
    **************************/
    // LED PIN init
    gpio_init(BUTTON_RED_PIN);
    gpio_set_dir(BUTTON_RED_PIN, GPIO_OUT);	
    gpio_init(BUTTON_GREEN_PIN);
    gpio_set_dir(BUTTON_GREEN_PIN, GPIO_OUT);	
    gpio_init(BUTTON_BLUE_PIN);
    gpio_set_dir(BUTTON_BLUE_PIN, GPIO_OUT);	

    // Setting up the i2c slave
    gpio_init(SLAVE_SDA);
    gpio_init(SLAVE_SCL);
    gpio_set_function(SLAVE_SDA, GPIO_FUNC_I2C);
    gpio_set_function(SLAVE_SCL, GPIO_FUNC_I2C);
    gpio_pull_up(SLAVE_SDA);
    gpio_pull_up(SLAVE_SCL);

    i2c_init(MODULE_I2C, I2C_BAUDRATE);
    i2c_slave_init(MODULE_I2C, I2C_SLAVE_ADDRESS, &i2c_slave_handler);

    // Setting up the i2c master
    gpio_init(MASTER_SDA);
    gpio_init(MASTER_SCL);
    gpio_set_function(MASTER_SDA, GPIO_FUNC_I2C);
    gpio_set_function(MASTER_SCL, GPIO_FUNC_I2C);
    gpio_pull_up(MASTER_SDA);
    gpio_pull_up(MASTER_SCL);

    i2c_init(PERIPHERAL_I2C, I2C_BAUDRATE);

    // Setting up the IRQ for the buttons to switch freq and to transmit
    gpio_pull_down(BUTTON);
    gpio_init(BUTTON);
    gpio_set_dir(BUTTON, GPIO_IN);

    gpio_set_irq_enabled_with_callback(BUTTON, GPIO_IRQ_EDGE_RISE  | GPIO_IRQ_EDGE_FALL, true, &button_event);

    SSD1306_init();
}

void init_render_area(render_area* area) {
    *area = (render_area){
        .start_col = 0,
        .end_col = SSD1306_WIDTH - 1,
        .start_page = 0,
        .end_page = SSD1306_NUM_PAGES - 1,
    };
    calc_render_area_buflen(area);

    // zero the entire display
    uint8_t buffer[SSD1306_BUF_LEN] = {0};
    render(buffer, area);
}

void color_button_led() {
    switch (button_color)
    {
    case BLUE:
        gpio_put(BUTTON_BLUE_PIN, 1);
        break;
    case WHITE:
        gpio_put(BUTTON_BLUE_PIN, 1);
    case YELLOW:
        gpio_put(BUTTON_GREEN_PIN, 1);
    case RED:
        gpio_put(BUTTON_RED_PIN, 1);
        break;
    default:
        break;
    }
}

void print_debug_data() {
    if (!DEBUG_MODE) return;
    sleep_ms(2000);
    printf(
        "Status:\n"
        "\tlit_frk: %s\n"
        "\tlit_car: %s\n"
        "\tbutton_color: %u, %s\n"
        "\tbutton_label: %d, %s\n"
        "\tbattery_count: %d\n"
        "\trelevant_rule: %u\n",
        lit_frk ? "true" : "false",
        lit_car ? "true" : "false",
        button_color,
        button_color == RED     ? "RED" :
        button_color == WHITE   ? "WHITE" :
        button_color == BLUE    ? "BLUE" :
        button_color == YELLOW  ? "YELLOW" : "BAD VALUE",
        button_label,
        BUTTON_LABELS[button_label],
        battery_count,
        relevant_rule);
}

uint8_t
get_relevant_rule() {
    if (button_color == BLUE && button_label == ABORT)
        return 1;
    if (battery_count > 1 && button_label == DETONATE)
        return 2;
    if (button_color == WHITE && lit_car)
        return 3;
    if (battery_count > 2 && lit_frk)
        return 4;
    if (button_color == YELLOW)
        return 5;
    if (button_color == RED && button_label == HOLD)
        return 6;
    return 7;
}

void
wait_for_data_from_master() {
    if (DEBUG_MODE) {
        battery_count = (get_rand_64() % (MAX_BATTERIES + 1)); // [0, 10]
        lit_frk = get_rand_64() % 2;
        lit_car = get_rand_64() % 2;

        i2c_state = RUNNING;
    }

    while (i2c_state != RUNNING) {
        sleep_ms(100);
    }

    relevant_rule = get_relevant_rule();

    print_debug_data();
}

void
check_releasing_held_button() {

}

void
check_press_immediate_release() {

}

void
check_relevant_rule() {
    switch (WIN_BY_RULE[relevant_rule]) {
        case RELEASE_HELD_BUTTON:
            check_releasing_held_button();
        break;
        case PRESS_IMMEDIATE_RELEASE:
            check_press_immediate_release();
        break;
        case INVALID_METHOD:
        default:
        break;
    }
}

void
write_center(render_area* frame_area, const char* str) {
    uint16_t centered_x = (SSD1306_WIDTH - strlen(str) * FONT_CHAR_WIDTH) / 2;
    uint8_t buffer[SSD1306_BUF_LEN] = {0};
    write_string(buffer, centered_x, 0, str);
    render(buffer, frame_area);
}

int main() {
    button_module_init();

    render_area frame_area;
    init_render_area(&frame_area);

    write_center(&frame_area, "LOADING");

    wait_for_data_from_master();

    write_center(&frame_area, BUTTON_LABELS[button_label]);

    color_button_led();

    while(i2c_state != SUCCEEDED && !lose_flag) {
    	// Main Loop
        check_relevant_rule();
    }

    if (i2c_state == SUCCEEDED) { // If there was a success, turn the LED GREEN and halt.
    	gpio_put(STATUS_GREEN_PIN, 1);
    	printf("Waiting forever\n");
    	while(true) sleep_ms(0);
    }

    if (lose_flag) {
    	gpio_put(STATUS_RED_PIN, 1);
    	while(true) sleep_ms(0);
    }
}
